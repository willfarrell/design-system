...
<svelte:head>
	...
	<meta name="description" content="The date input provides a native date picker for selecting calendar dates using type=date." />
</svelte:head>
...
<LayoutTableOfContents>
	...
	<Section>
		...
		<P>The date input provides a native date picker for selecting calendar dates. It uses <Code>type="date"</Code> which renders a platform-appropriate date picker widget.</P>
	</Section>
	<Section>
		<H2 id="example">Example</H2>
		<Snippet example="form-input-date" />
	</Section>
	<Section>
		<H2 id="accessibility">Accessibility</H2>
		<Ul>
			<Li>Use explicit <Code>&lt;label for="id"&gt;</Code> association.</Li>
			<Li>Use <Code>autocomplete="bday"</Code> or the appropriate token for the date type.</Li>
			<Li>Provide <Code>min</Code> and <Code>max</Code> attributes to constrain the valid range.</Li>
			<Li>Provide hint text describing the expected format via <Code>aria-describedby</Code>.</Li>
		</Ul>
	</Section>
	<Section>
