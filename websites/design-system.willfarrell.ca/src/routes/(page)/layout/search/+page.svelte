...
<svelte:head>
	<title>Search | Layout | Design System</title>
	...
	{#snippet aside()}
		<Search>
			<Form method="GET">
				...
				<P>Need help? <A href="/docs" target="_top">View our guidance</A></P>
			</Form>
		</Search>
	{/snippet}
		...
		<Section>
			...
			<Ul class="grid">
				{#each results as card}
					<Li><Card id={card.id}>
						<H2><A href={card.href} aria-describedby={card.id}>{card.title}</A></H2>
						{#if card.description}
