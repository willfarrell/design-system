...
test("WCAG 2.2 A 1.3.1 Info and Relationships — form has labels", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
	...
	const labels = form.first().locator("label");
	const count = await labels.count();
	expect(count).toBeGreaterThan(0);
});

test("WCAG 2.2 A 3.3.2 Labels or Instructions — form inputs have associated labels", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
	...
	const inputs = page.locator(
		'main form textarea, main form input:not([type="hidden"]), main form select',
	);
	...
	for (let i = 0; i < count; i++) {
		...
		expect(hasLabel, `input[${i}] should have an associated label`).toBe(true);
	}
});

test("WCAG 2.2 A 2.4.1 Bypass Blocks — has main landmark", async ({ page }) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
...
test("WCAG 2.2 A 4.1.2 Name, Role, Value — submit button has accessible name", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
	...
	const submitButtons = page.locator(
		'button[type="submit"], [is="ds-form-submit"] button',
	);
	...
	for (let i = 0; i < count; i++) {
		...
		expect(hasName, `submit button[${i}] should have an accessible name`).toBe(
			true,
		);
	}
});
	...
	const reordered = await page.$$eval("*", (els) =>
		els
			.filter((el) => {
				...
				return style.order !== "" && style.order !== "0";
			})
			.map(
				(el) =>
