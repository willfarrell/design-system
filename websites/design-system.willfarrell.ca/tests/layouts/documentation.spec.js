...
const path = "/layout/documentation";
...
test("WCAG 2.2 A 2.4.1 Bypass Blocks — has navigation landmarks (aside navs)", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
...
test("WCAG 2.2 AA 2.4.5 Multiple Ways — has table of contents navigation", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");

	const tocHeading = page.getByRole("heading", { name: "On this page" });
	await expect(tocHeading).toBeVisible();

	const tocLinks = page.locator("nav ol a[href^='#']");
	const count = await tocLinks.count();
	expect(count).toBeGreaterThan(0);
});

test("WCAG 2.2 A 1.3.1 Info and Relationships — nav elements have accessible labels", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
	...
	for (let i = 0; i < count; i++) {
		...
		if (count > 1) {
			expect(
				hasLabel,
				`nav[${i}] should have an accessible label when multiple navs exist`,
			).toBe(true);
		}
	}
});
