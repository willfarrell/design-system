...
const path = "/demo/accordion-chevron";
...
test("WCAG 2.2 A 4.1.2 Name, Role, Value — details/summary present", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
...
test("WCAG 2.2 A 2.1.1 Keyboard — toggle via keyboard", async ({ page }) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
