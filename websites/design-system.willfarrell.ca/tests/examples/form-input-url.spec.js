...
const path = "/demo/form-input-url";
...
test("WCAG 2.2 AA 1.3.5 Identify Input Purpose — input has autocomplete url", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");

	const inputs = page.locator('input[autocomplete*="url"]');
...
test("WCAG 2.2 A 3.3.2 Labels or Instructions — label present", async ({
	...
	const inputs = page.locator('input[type="url"], input');
	const count = await inputs.count();
	expect(count).toBeGreaterThan(0);
