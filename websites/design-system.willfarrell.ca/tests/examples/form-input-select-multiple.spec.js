...
const path = "/demo/form-input-select-multiple";
...
test("WCAG 2.2 A 4.1.2 Name, Role, Value — select has listbox role", async ({
	page,
}) => {
	await page.goto(path);
	await page.waitForLoadState("domcontentloaded");
...
test("WCAG 2.2 A 3.3.2 Labels or Instructions — label present", async ({
	...
	const selects = page.locator("select[multiple]");
